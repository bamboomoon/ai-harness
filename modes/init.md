...
4. **规则与风格**：写根与模块 AGENTS（必读 CONVENTIONS 提示、何时 | 读、用户确认过的工具表、纠正回路及其落点表、测试规则），复制 [../readable/](../readable/) 基线到 `.agents/skills/`，为每个模块建 `CONVENTIONS.md`（见 [../reference/style.md](../reference/style.md)，先用偏好清单填充），加 PR 模板。
