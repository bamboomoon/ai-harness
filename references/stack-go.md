...
| 声明顺序（Stepdown） | 基于 go/ast 的小工具（检查未导出函数位于首个调用者之后，带自动归位），见 [../assets/go/declorder/main.go](../assets/go/declorder/main.go) | 用户在意阅读顺序时加入 |
...
- gremlins `--diff` 没有任何结果｜Go 模块位于仓库子目录时路径匹配不到｜按包运行后自行按改动行过滤（参考实现 `assets/scripts/mutation.mjs` 已处理）｜gremlins 0.6.0，2026-09
