...
| agent 可用的 skill、MCP 与 CLI（只为判断是否需要用法规则，不列表） | 项目 `.mcp.json`、`.agents/skills`/`.claude/skills`、用户级 agent 配置（如 `~/.codex/config.toml` 的 `mcp_servers`、Claude Code 的 MCP 与 skill 目录）、项目文档提到的 CLI（如组件库、代码托管 CLI）、索引目录（如 `.codegraph/`） |
...
3. 问用户只有用户能回答的问题：使用哪些 agent（Claude Code、Codex…）；工具用法上有没有约定（如「必须用 X 不用 Y」），有则记下并落到对应 CONVENTIONS 或流程文档——不让用户逐项确认工具清单；最近对 agent 的 2~3 个真实纠正；代码风格偏好；约束（能否引入容器、CI 平台、可接受的门禁严格程度）。
   完成标准：纠正案例、偏好、约束与工具用法约定清单。
